class WiFeedback < ActiveRecord::Base
  ...
  named_scope :by_url, lambda { |url, userid| { :select => "wi_feedbacks.id,wi_feedbacks.positive_feedback,wi_feedbacks.negative_feedback", :include => :page, :conditions => ["pages.url = ? AND userid = ?", url, userid], :order => 'timestamp DESC', :limit => 1}}
